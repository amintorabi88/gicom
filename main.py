...
def get_git_diff():
    ...
    try:
        ...
        diff = subprocess.check_output(["git", "diff", "--cached"], stderr=subprocess.STDOUT).decode("utf-8").strip()
        return diff
...
@app.command()
def commit(
    context: str = typer.Option(None, "--context", "-c", help="Add extra context (e.g. 'Fixing login bug')"),
    dry_run: bool = typer.Option(False, "--dry-run", "-d", help="Print message but do not commit."),
    ...
    # 3. Display
    console.print(Panel(message, title="[bold blue]Generated Commit Message[/bold blue]", expand=False))
